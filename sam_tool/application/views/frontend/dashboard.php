...
?>
	...
	<script type="text/javascript">
		...
		$(window).scroll(function(){
			...
			if( position == bottom ){
				var row = Number($('#rowid').val());
				var allcount = Number($('#all_product_count').val());
				var rowperpage = 12;
				row = row + rowperpage;
				if(row <= allcount){
					$('.ajax-load').show();
					var url = "<?php echo base_url(); ?>sam/search_pagination";
					$('#row').val(row);
					$.ajax({
						url: url,
						type: 'post',
													...
													<input type="hidden" name="offset" id="offset" value="1">
															<div class="col-lg-2 col-md-2 col-sm-6 col-xs-12">	
																...
																<div class="form-group">
																	<select name="state_name"  id="state_name"  class="form-control selectsearch"  onChange="statecategorylist(this.value);">
															...
															<div class="col-lg-2 col-md-2 col-sm-6 col-xs-12" >	
																...
																<div class="form-group">
																	<select name="city_name"  id="city_name"  class="form-control selectsearch" onChange="citycatelist(this.value);">
							...
							<a href="<?= base_url('frontend/property_details')?>" style="margin-bottom:1px;" id="dixc">
								<div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
									<div class="dashboard-stat" style="border:solid 1px;" type_id="<?php echo $result['type_id']; ?>" propert_id="<?php echo $result['propert_id']; ?>">
										...
										<div class="details">
											<div class="number">
											<span data-counter="counterup"><?php echo $results['count'] ?></span>
												<?php echo $result['property_type']; ?> in 
...
<script>	
	function statecategorylist(state_name){

		var url="<?php echo base_url.'frontend/getCity/';?>";	
		$("#city_name").load(url,{'state_name':state_name},function(res){
			//alert(res);	
		});	
	}

	function citycatelist(cat_id){

		var url="<?php echo base_url.'frontend/getAddresss/';?>";	
		$("#area_name").load(url,{'city_name':city_name},function(res){
			alert(res);	
		});	
	}
$(document).ready(function() {
